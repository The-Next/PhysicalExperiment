...
from Spectrometer.serializers import *
from Spectrometer.models import *
from Spectrometer.utils import Spectrometermain
from rest_framework import viewsets
# Create your views here.

class SpectometerAPI(viewsets.ModelViewSet):
    '''
    retrieve:
            根据实验数据主键获得信息
    '''
    queryset = Spectrometer.objects.all().order_by('-pk')
    serializer_class = SpectrometerSerializers
    permission_classes = (AllowAny,)  # 该接口权限为任意用户

    def create(self, request, *args, **kwargs):
        '''提交数据验证并保存'''
        data = request.data
        anwser = Spectrometermain.Spectrometer(data)
        serializer = SpectrometerSerializers(data=anwser)
    ...
    def getListById(self,request, *args, **kwargs):
        ...
        pk = kwargs['pk']
        serializer = SpectrometerSerializers(Spectrometer.objects.filter(user_id__exact=pk),many=True)
